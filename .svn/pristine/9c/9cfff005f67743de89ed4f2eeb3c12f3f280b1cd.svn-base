...
public class InstinctMobileWork {
	...
	public void everydayUploadInstinct(){
		log.info("=========InstinctMobileWork.everydayUploadInstinct()开始执行=============");
		...
		try {
			...
			if (localhostIp.equals(serverIp)) {
				...
				list = instinctDao.getNeedExportInfo("instinct.queryMobileInfo");
				...
				if(flag){
					...
					for(ThirdinfoModel thirdinfoModel:list){
						instinctDao.updateInstinctFlag(thirdinfoModel, "instinct.updataMobileFlag");
					}
				}else{
		...
		log.info("InstinctMobileWork.everydayUploadInstinct()执行完成..用时：" + (end - start) + "m");
	}
}
