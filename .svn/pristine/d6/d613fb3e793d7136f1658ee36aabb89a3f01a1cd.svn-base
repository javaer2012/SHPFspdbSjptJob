...
public class InstinctWHWork {
	private static final Logger log = Logger.getLogger(InstinctWHWork.class);
	@Inject
	...
	public void everydayUploadInstinct(){
		...
		try {
			...
			if (localhostIp.equals(serverIp)) {
				...
				list = instinctDao.getNeedExportInfo("instinct.getNeedWHList");
				...
				if(flag){
					...
					for(ThirdinfoModel thirdinfoModel:list){
						instinctDao.updateInstinctFlag(thirdinfoModel, "instinct.updataWHFlag");
					}
				}else{
