...
public class InstinctIdWork {
	private static final Logger log = Logger.getLogger(InstinctIdWork.class);
	@Inject
	...
	public void everydayUploadInstinct(){
		log.info("=========InstinctIdWork.everydayUploadInstinct()开始执行=============");
		...
		try {
			...
			if (localhostIp.equals(serverIp) ) {
				...
				list = instinctDao.getNeedExportInfo("instinct.queryIDInfo");
				...
				if(flag){
					...
					for(ThirdinfoModel thirdinfoModel:list){
						instinctDao.updateInstinctFlag(thirdinfoModel, "instinct.updataIDFlag");
					}
				}else{
		...
		log.info("InstinctIdWork.everydayUploadInstinct()执行完成..用时：" + (end - start) + "m");
	}
}
