...
public class ApprovalIdWork {
	private static final Logger log = Logger.getLogger(ApprovalIdWork.class);
	@Inject
	...
	public void everydayUploadApproval() {
		log.info("=========ApprovalIdWork.everydayUploadApproval()开始执行=============");
		...
		try {
			...
			if (localhostIp.equals(serverIp)) {
				...
				list = approvalDao.getNeedExportInfo("approval.getNeedIDmobileList");
				...
				if(flag){
					...
					for(ThirdinfoModel thirdinfoModel:list){
						approvalDao.updateApprovalFlag(thirdinfoModel, "approval.updataIDmobileFlag");
					}
				}else{
		...
		log.info("ApprovalIdWork.everydayUploadApproval()执行完成..用时：" + (end - start) + "m");
	}
}
