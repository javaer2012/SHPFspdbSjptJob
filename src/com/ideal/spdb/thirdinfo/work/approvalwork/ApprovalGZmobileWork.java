...
public class ApprovalGZmobileWork {
	private static final Logger log = Logger.getLogger(ApprovalGZmobileWork.class);
	@Inject
	...
	public void everydayUploadApproval() {
		log.info("=========ApprovalGZmobileWork.everydayUploadApproval()开始执行=============");
		...
		try {
			...
			if (localhostIp.equals(serverIp)) {
				...
				list = approvalDao.getNeedExportInfo("approval.getNeedGZmobileList");
				...
				if(flag){
					...
					for(ThirdinfoModel thirdinfoModel:list){
						approvalDao.updateApprovalFlag(thirdinfoModel, "approval.updataGZmobileFlag");
					}
				}else{
		...
		log.info("ApprovalGZmobileWork.everydayUploadApproval()执行完成..用时：" + (end - start) + "m");
	}
}
