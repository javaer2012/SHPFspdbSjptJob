...
public class InstinctSZWork {
	private static final Logger log = Logger.getLogger(InstinctSZWork.class);
	@Inject
	...
	public void everydayUploadInstinct(){
		...
		try {
			...
			if (localhostIp.equals(serverIp)) {
				...
				list = instinctDao.getNeedExportInfo("instinct.getNeedSZList");
				...
				if(flag){
					...
					for(ThirdinfoModel thirdinfoModel:list){
						instinctDao.updateInstinctFlag(thirdinfoModel, "instinct.updataSZFlag");
					}
				}else{
