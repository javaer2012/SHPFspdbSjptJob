...
public class InstinctZfbWork {
	private static final Logger log = Logger.getLogger(InstinctZfbWork.class);
	@Inject
	...
	public void everydayUploadInstinct(){
		log.info("=========InstinctZfbWork.everydayUploadInstinct()开始执行=============");
		...
		try {
			...
			if (localhostIp.equals(serverIp)) {
				...
				list = instinctDao.getNeedExportInfo("instinct.queryZfbInfo");
				...
				if(flag){
					...
					for(ThirdinfoModel thirdinfoModel:list){
						instinctDao.updateInstinctFlag(thirdinfoModel, "instinct.updataZfbFlag");
					}
				}else{
		...
		log.info("InstinctZfbWork.everydayUploadInstinct()执行完成..用时：" + (end - start) + "m");
	}
}
